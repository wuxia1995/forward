...
public class MethodUtil {
	...
	private Logger logger = Logger.getLogger(MethodUtil.class);
	private FileItemFactory factory = new DiskFileItemFactory();
	public synchronized String requestForword(HttpServletRequest request,
			...
			try {
				...
				while(iterator.hasNext()) {
					...
					if(item.isFormField()) {
						...
						logger.info("byte******:"+item.getSize());
						logger.info("byte.length:"+pic.length);
					/*	byte[] b = new byte[(int) item.getSize()];
						inputStream.read(b);*/
						logger.info("file-----"+filedName+":"+fileName);
		...
		//转发和设定报头信息
		logger.info("uir:"+request.getRequestURI());
		...
		String SDKreply = new HttpUploadFile().forwardRequest(header, param, file, meta);
